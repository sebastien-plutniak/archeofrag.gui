...
ui <- shinyUI(fluidPage(  # UI ----
                ...
                <p>
                <h3>Aims</h3>
                 <i>archeofrag.gui</i> aids in assessing the <b>distinctions</b> between observed <b>archaeological spatial units</b> (e.g. layers, pits, etc.) and their <b>formation process</b> by examining the refitting relationships between fragments of objects contained in these units.
                 <ul>
                  <li>How relevant is it to differentiate layers A and B? What about layers B and C?</li>
                  <li>Should layers A and B, or  B and C, be merged? </li>
                  <li>Were layers A and B generated by one or two deposition events? Did transport occurred from A to B or conversely?</li>
                 </ul>
                This application complements and draws upon the <i><a href=https://cran.r-project.org/web/packages/archeofrag/index.html target=_blank>archeofrag</a></i> R package, which implements the TSAR method (Topological Study of Archaeological Refitting). The TSAR method addresses more than the quantity of refitting relationships to also consider their relative distribution and <b>topology</b>. 
                </p>
                <p>
                ...
                <h3>Input Data</h3>
                <p>
                  Use the menu on the left to upload your 'relationships' and 'fragments' data as CSV files. 
                  <ul>
                  ...
                  Alternatively, explore 'Rubish Site' playground data or load <b>real-world</b> archaeological datasets (see details in the 'Datasets' tab).
                </p>
                <h3>Variable selections</h3>
                  <p>By default, the full fragmentation graph is studied. However, in the left menu select:
                     <ul>
                      <li>the <b>spatial variable</b>  to consider (i.e. the spatial unit containing the fragments),</li>
                      ...
                      <li>Optionally, the dataset can be subsetted by selecting a variable and some of its values.</li>
                    </ul>
                  </p>
                <h3>Measurements</h3>
                ...
                <ol>
                  <li> for the <b>fast exploration</b> of multiple options to merge those spatial units,</li>
                  <li>  <b>modifying</b> the dataset by merging a selection of spatial units (which are then available from <i>archeofrag.gui</i>'s functionalities).</li>
                </ol>  
                  </p>
                                                  ...
                                                  "<div style=width:40%;, align=left>
                                                   ...
                                                   HTML("
                                                        <ul>
                                                          ...
                                                          <li><b>Cohesion</b>:
                                                            <ul>
                                                              <li>for a pair of spatial units, the measure of the consistency of each unit, how it is 'self-adherent' to itself (see <a href=https://doi.org/10.1016/j.jas.2021.105501 target=_blank>Plutniak 2021</a>)</li>
                                                              <li>for a single spatial unit, cohesion is computed by comparing the unit to its minimized copy (i.e. having the same number of fragments but minimal number of refits).  Weighting options are supported.</li>
                                                              </ul>
                                                            </li>
                                                          <li><b>Cohesion diff.(erence)</b>: for a pair of spatial units, highest cohesion value minus lowest cohesion value. (See the 'Spatial units optimisation' tab for details.) </li>
                                                        </ul>
                                                        "),
                                                   ), # end column
                                                   column(10, align="center",
                                          ...
                                          tabPanel("Visualisation", # VISUALISATION ----
                                                   fluidRow(
                                                     ...
                                                     column(10, align="center",
                                                       ...
                                                       HTML("</div>"),
                                                       br(),
                                                       sliderInput("node.size", "Node size", min=1, max=6, value=3),
                                                       br(),
                                          ...
                                          tabPanel("Spatial units optimisation", # SPATIAL UNITS OPTIMISATION ----
                                                   fluidRow(
                                                     ...
                                                     column(10, align="center",
                                                            ...
                                                            <h2>Presentation</h2>
                                                            <p>
                                                                 It happens that archaeological spatial units (e.g.  stratigraphic layers) observed on the field reveal to be merged during post-excavation analysis. But which ones? <i>archeofrag.gui</i> helps you to determine merging solutions that result in more balanced spatial units series. Here 'balanced' refers to the distribution of archaeological information (i.e. refits number and topology) in the series of spatial units to support their individual archaeological relevance</p>
                                                                 <p> An ideal situation, where equal information is known about a series of spatial units would result in pairwise cohesion values = 0.5 and admixture = 0. In such a case, for every pair of units, the <b>difference</b> between the two cohesion values would be 0. Consequently, looking for spatial divisions minimising the difference between pairs of cohesion values reveal archaeologically relevant merging solutions of spatial units. 
                                                                 </p>
                                                                 <p>
                                                                 This tool 1) determines the series of possible spatial units merging, 2) generates the corresponding fragmentation graphs, 3) computes the cohesion value of each unit for all possible pairs of spatial units (as in the 'Measurements' tab), 4) summarises these values by measuring their median and <a href=https://en.wikipedia.org/wiki/Median_absolute_deviation target=_blank>median absolute deviation</a>.</p>
                                                                 <h2>Instructions</h2>
                                                                   <ul>
                     ...
                     <li>whether the observed value is lower / within / higher than  the interquartile range of values simulated for H1 and H2, respectively ('Obs. value/H1' and 'Obs. value/H2' columns).</li>
                      ...
                      <li>The distributions of values for each hypothesis are represented by dark (H1) and light (H2) grey shades, respectively (except for cohesion).</li>
                                                  ...
                                                  fluidRow(
                                                    ...
                                                    column(10, align="center",
                                                           HTML("<div style=width:40%;, align=left><p>
                                                                The number of connection relationships (edges of the graph) is not constrained in this use of the simulator. Consequently, the edge count variability can be used to compare the empirical and simulated fragmentation graphs.
                                                                </p></div>")
                                                    )),
                                                  fluidRow(column(10,
                ...
                <ul>
                  <li><b>Plutniak, S. 2025b</b>. '<a href=https://hal.archives-ouvertes.fr/hal-05352218 target=_blank>Evaluer les unit&#233;s spatiales et analyser la formation des ensembles arch&#233;ologiques &#224; partir des remontages : l'application archeofrag.gui, une cha&#238;ne op&#233;ratoire int&#233;gr&#233;e</a>', <i>BSPF</i>, 122 (3), p. 489-493.</li>
                ...
                <h2>Datasets</h2> 
                <ul>
                  <li><b>Rue Auguste Isaac 35</b>:  Pasty, J.-F. 2026. <i>Fouille du 35 rue Auguste Isaac (Lyon, France). Donn&#233;es lithiques des occupations &#233;pipal&#233;olithiques et m&#233;solithiques</i>,  doi: <a href=https://doi.org/zenodo.22663886 target=_blank>zenodo.22663886</a>.</li>
                  <li><b>Barger Gulch</b>:  Surovell T. A. 2020.  <i>Barger Gulch Locality B database</i>, (tDAR id: 458541), doi: <a href=https://doi.org/10.48512/XCV8458541 target=_blank>10.48512/XCV8458541</a>.</li>
                  ...
                  <li><b>Chauzeys</b>: Chadelle J.-P. (dir.),  M. Baillet, A. Barbeyron, M. Brenet, H. Camus, E. Claud, F. Jude, S. Kreutzer, A. Michel,  N. Mercier, M. Rabanit, S. Save, F. Sellami, A. Vaughan-Williams. 2021. <i>Chauzeys, Saint-M&#233;dard-de-Mussidan (Dordogne, Nouvelle-Aquitaine)</i>, Excavation report, Perigueux. </li>
                  <li><b>Cuzoul de Gramat</b>: Gardeur M. 2025. 'Bone refits from the Cuzoul de Gramat Mesolithic layers (archaeological site, France)', <i>Zenodo</i>, doi: <a href=https://doi.org/10.5281/zenodo.14975910 target=_blank>10.5281/zenodo.14975910</a>.</li>
                  ...
                  <li><b>Etiolles D71</b>: Caron-Laviolette E. 2026. 'Lithic data from the D71 dwellings at Etiolles palaeolithic site (stone industries description and refitting)', <i>Zenodo</i>, doi: <a href=https://doi.org/10.5281/zenodo.22663797 target=_blank>10.5281/zenodo.22663797</a>.</li>
                  ...
                  <li><b>Grande Rivoire 1st Meso</b>: Derbord L., A. Angelin. 2025. 'Mesolithic artefact refitting data from La Grande Rivoire (Sassenage, Is&#232;re)', <i>Zenodo</i>, doi: <a href=https://doi.org/10.5281/zenodo.15289796 target=_blank>10.5281/zenodo.15289796</a>.</li>
                  <li><b>Grande Rivoire 2nd Meso</b>: Derbord L., A. Angelin. 2025. 'Mesolithic artefact refitting data from La Grande Rivoire (Sassenage, Is&#232;re)', <i>Zenodo</i>, doi: <a href=https://doi.org/10.5281/zenodo.15289796 target=_blank>10.5281/zenodo.15289796</a>.</li>
